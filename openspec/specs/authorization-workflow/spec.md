# authorization-workflow Specification

## Purpose

Define el flujo **concreto de aprobación de gastos** de ToyPOS: al registrarse un gasto nace una **aprobación** `pendiente` que transiciona a `aprobada`, `rechazada` o `cancelada` (estados terminales e inmutables). El aprobador se determina por permiso `autorizar_gasto` + alcance que cubra la sucursal —una **autoridad administrativa, nunca del cajero**—, con **segregación de deberes** (el solicitante no resuelve lo suyo, salvo auto-aprobación administrativa explícita). La resolución determina si el gasto cuenta como autorizado en el corte, respetando la inmutabilidad de cortes ya cerrados (conciliación posterior). La máquina de estados se mantiene reutilizable; se generaliza solo si aparece un segundo flujo real de aprobación (KISS/YAGNI).
