...
Establece la organización y el control de acceso de ToyPOS: **sucursales** (una `matriz` y varios `expendio`), **usuarios** que se autentican localmente, y un **RBAC modular** de permisos atómicos componibles en roles a medida. El **alcance** del usuario (una, varias o todas las sucursales) es el mecanismo de **aislamiento por diseño**: nadie lee ni modifica datos fuera de su alcance. Incluye la **bitácora de auditoría** transversal —toda operación mutante queda atribuida a su actor (usuario o `sistema`)— y la regla de tiempo unificado (UTC en el nodo, presentado en hora local de la sucursal).
