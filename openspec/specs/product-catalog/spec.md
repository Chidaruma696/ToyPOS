...
Define el **catálogo de productos global** de ToyPOS, gobernado por la matriz y disponible para todas las sucursales. Cada producto se tipa por su **unidad de venta** (`peso_variable` por kilogramo o `pieza` por unidad a precio fijo), con el **origen** como eje independiente (producido por matriz o comprado a proveedor externo). Los productos `pieza` portan un código de barras único —GTIN de fábrica para los comprados o EAN-13 decodable **generado por matriz** para los producidos—. El inventario por sucursal vive en la capacidad `inventory`; aquí vive solo el catálogo.
