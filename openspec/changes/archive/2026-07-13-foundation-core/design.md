...
- *Motivación:* en sistemas sin auditoría un empleado puede desajustar el inventario sin que haya forma de probar quién fue.
...
Cada **documento generado** (nota de venta, corte de caja, gasto; a futuro envío, pedido, devolución) lleva —además de su `uuid` técnico (D12)— un **folio** legible: `{código_sucursal}{tipo}{consecutivo}` **todo junto, sin separadores**, p. ej. `SROC56` (corte 56 de Santa Rosa), `SRON13280` (nota), `SROG204` (gasto). El folio es lo que la gente **ve, imprime y cita**; el `uuid` es solo para la máquina y el sync. Solo los *documentos/transacciones* llevan folio — el catálogo (producto, rol, usuario) no.
- **Código de sucursal:** atributo **explícito, único e inmutable** que el admin asigna al crear la sucursal (2–4 caracteres alfanuméricos, p. ej. `SRO`, `SRI`, `VAL`). **No se deriva del nombre** —así Santa Rosa y Santa Rita no colisionan— y al ser inmutable, renombrar la sucursal no altera folios históricos. El sistema rechaza códigos duplicados.
- **Consecutivo por (sucursal, tipo):** cada tipo de documento lleva su **propia secuencia en cada sucursal**; por eso Santa Rosa va en el corte #56 y Valle en el #19 sin relación. Se genera **local** en el nodo (una caja por sucursal = un solo escritor, D13) → sin coordinación central, offline-safe, y globalmente único porque el código de sucursal no colisiona.
...
- *Alternativa descartada:* prefijo derivado del nombre → colisiona (Santa Rosa/Santa Rita) y se rompe al renombrar.
...
- *El bug que evitamos:* si un servidor remoto en otro huso sella la hora, el log dice 14:56 cuando eran las 10:00, y con eso nada es rastreable. Aquí el **nodo** sella el instante y la conversión a hora local es determinista.
